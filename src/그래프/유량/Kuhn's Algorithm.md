...
다음과 같은 이분 그래프가 있다고 하자. 이분 그래프는 정점을 왼쪽 그룹과 오른쪽 그룹으로 나누었을 때 간선이 항상 서로 다른 그룹 사이에만 존재하는 그래프이다.

초기 상태에서는 두 개의 배열을 사용해 각 정점의 매칭 상태를 관리한다고 가정한다. 이 그래프에서 **서로 겹치지 않는 왼쪽 정점–오른쪽 정점 쌍의 최대 개수**를 구하는 것이 목표이다.

![](https://github.com/user-attachments/assets/64fa9a91-c479-43fd-91a8-01d0147d19a3)

먼저 **A1**에 대해 연결 가능한 첫 번째 정점인 **B1**과 매칭을 시도한다. 현재 B1은 어떤 정점과도 매칭되어 있지 않으므로 **A1–B1 매칭**이 생성된다.

![](https://github.com/user-attachments/assets/e797957b-fcdc-4bbd-9e08-cdf69a2571e8)

다음으로 **A2**에 대해 연결 가능한 첫 번째 정점인 **B3**과 매칭을 시도한다. B3 역시 아직 매칭되어 있지 않으므로 **A2–B3 매칭**이 생성된다.

![](https://github.com/user-attachments/assets/3656594b-fc8d-44a3-bdf9-05d81dab8e7b)

이제 **A3**에 대해 연결 가능한 첫 번째 정점인 **B1**과 매칭을 시도한다. 하지만 B1은 이미 **A1**과 매칭되어 있으므로 바로 매칭할 수 없다.

![](https://github.com/user-attachments/assets/f061def3-b7c5-46ba-ad35-2ad6faeaa7d7)

이 경우 기존 매칭을 재배치할 수 있는지 확인한다. B1과 연결되어 있던 **A1**이 다른 정점과 매칭될 수 있는지 탐색한다. A1은 **B2**와 연결할 수 있으므로 매칭을 **A1–B2**로 변경한다. 그 결과 **A3–B1 매칭**이 새롭게 가능해진다.

![](https://github.com/user-attachments/assets/9deda089-4d71-4ba0-b037-a5bf49204326)

마지막으로 **A4**에 대해 연결 가능한 첫 번째 정점인 **B3**과 매칭을 시도한다. 그러나 B3은 이미 **A2**와 매칭되어 있다. A2가 다른 정점과 매칭될 수 있는지 확인하지만 가능한 정점이 없으므로 **A4는 매칭에 실패한다.**

이와 같이 Kuhn's Algorithm은 기존 매칭을 재배치하면서 새로운 매칭을 확장해 나가며 **최대 매칭**을 찾는다.
