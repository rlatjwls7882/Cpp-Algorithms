...
DSU(Disjoint Set Union)는 서로소 집합을 관리하기 위한 자료구조로, \textbf{Find} 연산과 \textbf{Union} 연산을 제공한다.

\textbf{Find} 연산은 특정 원소가 속한 집합의 대표 원소를 찾는 연산이며, 이 과정에서 경로 압축을 적용할 수 있다.

\textbf{Union} 연산은 두 원소가 속한 집합의 대표 원소를 찾아 두 집합을 하나로 합치는 연산이다.
...
이를 방지하기 위해 \textbf{경로 압축}을 사용해야 한다. 경로 압축은 Find 연산을 수행하는 과정에서 탐색 경로에 있는 모든 노드가 직접 대표 원소를 가리키도록 만드는 방법이다.
