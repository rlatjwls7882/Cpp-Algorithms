...
* 그래프
  ...
  * 최단 경로
    * Dijkstra's Algorithm (다익스트라) 🟡 Gold IV
    * Bellman-Ford Algorithm (벨만-포드) 🟡 Gold IV
    * Floyd-Warshall Algorithm (플로이드-와샬) 🟡 Gold IV
  ...
  * 최소 스패닝 트리
    * Kruskal’s Algorithm (크루스칼) 🟡 Gold IV
  * 유량
    * Kuhn's Algorithm (Maximum Bipartite Matching, 이분 매칭) 🟢 Platinum IV
    * Edmonds-Karp Algorithm (애드몬드-카프) 🟢 Platinum IV
    * Dinic's Algorithm (디닉) 🟢 Platinum II
    * MCMF (Min Cost Max Flow) Algorithm (SPFA 구현) 🟢 Platinum III
    * Hopcroft-Karp Algorithm (호프크로프트-카프) 🟢 Platinum III
  * 컴포넌트 분해
    * Tarjan’s Algorithm (SCC, 타잔) 🟢 Platinum V
  ...
  * 세그먼트 트리
    ...
    * Dynamic Segment Tree (동적 세그) 🟢 Platinum V
    * Segment Tree with Lazy Propagation (레이지 세그) 🟢 Platinum IV
...
* 수학
  ...
  * 기하
    ...
    * Line Intersection (선분 교차 판정) 🟡 Gold II
...
* 스위핑
  * Sweeping Algorithm (스위핑) 🟡 Gold V
  * Imos Method (いもす法, 이모스법, Difference Array Trick, 차분 배열 트릭) 🟢 Platinum IV
* DP (동적 프로그래밍)
  * TSP (Traveling Salesman Problem, 외판원 순회 문제) 🟡 Gold I
  * Deque Trick (덱 트릭) 🟢 Platinum V
  * Convex Hull Trick (볼록 껍질 최적화) 🟢 Platinum II
* 쿼리 처리
  * Offline Query (오프라인 쿼리) 🟢 Platinum IV
  * Square Root Decomposition (제곱근 분할법) 🟢 Platinum II
  * Mo's Algorithm (모스) 🟢 Platinum II
...
## Offline Query (오프라인 쿼) 🟢 Platinum IV
쿼리를 미리 저장한 뒤 순서를 재정렬하여 효율적으로 처리하는 테크닉

시간복잡도: 문제마다 다름

## Square Root Decomposition (제곱근 분할법) 🟢 Platinum II
배열을 $\sqrt{N}$ 크기의 블록으로 분할하여 구간 쿼리를 $O(\sqrt{N})$에 처리하는 알고리즘

시간복잡도: $O(Q\sqrt N)$ ($Q$: 쿼리의 수)

## Mo's Algorithm (모스) 🟢 Platinum II
쿼리를 제곱근 분할 기준으로 정렬하여 구간을 이동하며 처리하는 알고리즘

시간복잡도: $O(Q\sqrt N)$

## PBS (Parallel Binary Search, 병렬 이분 탐색) 🟢 Platinum I
여러 쿼리를 동시에 이분 탐색하여 같은 mid에 대해 한 번만 검증하는 기법

시간복잡도: $O((N+Q)\log C)$ ($C$: 최대 이분탐색 범위)
